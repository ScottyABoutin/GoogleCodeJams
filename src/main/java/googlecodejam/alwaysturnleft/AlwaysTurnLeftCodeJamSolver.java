...
/**
 * An {@code AlwaysTurnLeftCodeJamSolver} solves the "Always Turn Left" Code Jam.
 * <p>
 ...
 * @version 1.0.0
 */
public class AlwaysTurnLeftCodeJamSolver extends GoogleCodeJamSolver {
  
  /**
   * Used to interpret input for traversing the maze.
   */
  private CodeJamInputInterpreter inputInterpreter;
  /**
   * Used to interpret the character that represents a particular {@code Coordinate}.
   */
  private CoordinateInterpreter coordinateInterpreter;
  
  /**
   * Creates an {@code AlwaysTurnLeftCodeJamSolver} that uses the specified strategy to solve this
   * Code Jam. After this is constructed, {@link #go(String[])} should be called on this object to
   * solve the Code Jam.
   * 
   * @param mazeConstructorStrategy The strategy used to build the maze.
   */
  public AlwaysTurnLeftCodeJamSolver(MazeConstructorStrategy mazeConstructorStrategy) {
    inputInterpreter = new CodeJamInputInterpreter(mazeConstructorStrategy);
    coordinateInterpreter = new CoordinateInterpreter();
  }
  
  /**
   * Amends the default case number format by adding a new line after the colon rather than the
   * space.
   */
  @Override
  protected String caseNumber(int testNumber) {
    StringBuilder builder = new StringBuilder(super.caseNumber(testNumber));
    builder.replace(builder.length() - 1, builder.length(), lineSeparator());
    return builder.toString();
  };
  
  /**
   * Solves a single test case of the Always Turn Left Code Jam.
   */
  @Override
  protected String solve(Scanner in) throws IOException {
    String testCase = in.nextLine();
    PerfectMaze maze = inputInterpreter.interpret(testCase);
    
    StringBuilder messageBuilder = new StringBuilder();
    int currentColumn = 0;
    final int numColumns = maze.getNumColumns();
    for (CoordinateWalls coordinate : maze) {
      messageBuilder.append(coordinateInterpreter.interpret(coordinate));
  ...
  /**
   * Runs the {@code AlwaysTurnLeftCodeJamSolver} application.
   * 
  ...
  public static void main(String[] args) throws IOException {
    MazeConstructorStrategy strategy =
                                     ThreadLocalRandom.current()
                                                      .nextBoolean() ? new MazeWithoutWallsConstructorStrategy()
                                                                     : new MazeWithWallsConstructorStrategy();
    new AlwaysTurnLeftCodeJamSolver(strategy).go(args);
  }
}